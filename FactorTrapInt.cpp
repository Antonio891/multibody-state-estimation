	...
	const std::string & s, const gtsam::KeyFormatter & keyFormatter) const
{
	std::cout << s << "FactorTrapInt(" << keyFormatter(this->key1()) << ","
		<< keyFormatter(this->key2()) << "," << keyFormatter(this->key3()) << "," << keyFormatter(this->key4()) << ")\n";
	...
	boost::optional<gtsam::Matrix&> H3, boost::optional<gtsam::Matrix&> H4) const
{
	...
	gtsam::Vector err =
		x_kp1.vector() - x_k.vector() – 0.5 * timestep_ * v_k.vector() - 0.5 * timestep_ * v_kp1.vector();

	\\ Jacobian of “err” respect to[x_k x_kp1 v_k v_kp1]
		if (H1) \\ If the Jacobian derr / dx_k is provided take H1v instead of x_k
		{
			auto & H1v = H1.value();
			H1v = -Eigen::MatrixXd::Identity(n, n);
		}
			if (H2) \\ If the Jacobian derr / dx_kp1 is provided take H2v instead of x_kp1
			{
				auto & H2v = H2.value();
				H2v = Eigen::MatrixXd::Identity(n, n);
			}
				if (H3) \\ If the Jacobian derr / dv_k is provided take H3v instead of v_k
				{
					auto & H3v = H3.value();
